    ...
    <button className="Collapse w-full group" onClick={() => handleClick()}>
      <div className="border-b border-current w-full py-1 md:py-3 xl:py-5 overflow-hidden">
        <div className="relative overflow-hidden py-3">
          <motion.div variants={reveal} className="flex flex-wrap items-start group">
            <span className={`block text-xs mr-4 w-full md:w-auto text-left mb-2 md:mb-0 md:mt-px xl:mt-1`}>{ index }</span>
            <span className={textLarge ? `block text-2xl md:text-2xl xl:text-3xl text-left font-display leading-extratight mb-0 pb-0 md:-mb-3 group-hover:line-through w-8/12 md:w-7/12 lg:w-8/12` : `block text-2xl md:text-2xl text-left font-display leading-extratight mb-0 pb-0 md:-mb-2 w-8/12 md:w-7/12 lg:w-8/12 group-hover:line-through`}>{ heading }</span>
