...
const PrivacyPage = ({ data: { studio, entry }, location }) => {
  return (
    <>
      ...
      />
        ...
        >
          ...
          <header className="p-4 pb-0 md:p-6 md:pb-0 fixed md:hidden fixed-when-scroll top-0 left-0 right-0 h-14 md:h-22 z-50 flex flex-wrap">
        ...
        <div className="h-14 md:h-22 z-10 hidden md:block fixed top-0 left-0 bg-white w-full" data-scroll-sticky data-scroll data-scroll-target="#___gatsby"></div>

        <div className="h-14 md:h-22 z-30 fixed block md:hidden top-0 left-0 bg-white w-full fixed-when-scroll"></div>
          ...
          >
            <motion.div variants={fade}>
              <div className="content content--fancy text-lg leading-tight w-10/12 md:w-full md:max-w-lg xl:max-w-2xl" dangerouslySetInnerHTML={{ __html: entry.content }}></div>
            </motion.div>
          </motion.div>
...
export const query = graphql`
  query PrivacyEntryQuery {
    entry: datoCmsPrivacyPolicy {
      title
      content
      slug
    }
